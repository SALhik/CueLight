...
**HTTP auth:** when a join password is set, the mutating endpoints (`POST /api/showfile/{f}`, `POST /api/patch/{f}` including `_probe_test`, `POST /api/resume_show`) and `GET /api/showlog` / `GET /api/showreport` require it — `X-CueLight-Password` header or `?password=` query param (kept for curl/direct-URL use), checked by `_password_ok()` in main.py; wrong/missing → 401. The caller sends the header from `full_state` (`pwAuth()` in caller.js — the log/report buttons fetch with it and open blob URLs, so the password never appears in a URL); the editor prompts on the first 401 and keeps it in `sessionStorage` (`authFetch()` in editor.js). `POST /api/_test_reset` answers 404 unless the `CUELIGHT_TEST_MODE=1` env var is set — it must never be callable in production.
